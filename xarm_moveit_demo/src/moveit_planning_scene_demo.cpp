...
int main(int argc, char *argv[])
{
    ...
    arm.setNamedTarget("Home"); // 回到home位置
    arm.move();
    ...
    shape_msgs::msg::SolidPrimitive table_primitive; // 基本几何形状的消息类型
    // 几何形状三个属性，形状、尺寸、位置
    ...
    table_object.primitives.push_back(table_primitive); // 往话题中添加基本几何形状
    table_object.primitive_poses.push_back(table_pose); // 往话题中添加基本几何形状的位姿
    table_object.operation = table_object.ADD;          // 添加物体
    ...
    // 将桌面、长方体添加到规划场景中
    scene.addCollisionObjects(collision_objects);   // 先添加到缓存
    scene.applyCollisionObjects(collision_objects); // 再立即应用
